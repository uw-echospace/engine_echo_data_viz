...
# with open("time_index.json") as f:
#     zarr_index = json.load(f)

# HIGHRES_DATA_DIR = "/home/exouser/xylin01/noaa-2021"
...
def load_dataset():
    ...
    if mvbs_dataset is None:
        try:
            ...
            mvbs_dataset = xr.open_dataset("merged_600s_v2.nc")
...
def load_highres_dataset():
    global highres_dataset
    if highres_dataset is None:
        try:
            print("Loading merged 60s (high-res) dataset...")
            highres_dataset = xr.open_dataset("merged_60s.nc")
            print("High-res dataset loaded")
        except Exception as e:
            print(f"Error loading high-res dataset: {e}")
            highres_dataset = None
...
def get_echogram():
    ...
    try:
        ...
        ds_highres = xr.open_dataset("/home/exouser/xylin01/mapapp/merged_60s.nc")
        print("High-res dataset loaded")

        # Get channel name
        channels = ds_highres.channel.values
        ...
        if start_time and end_time:
            try:
                ...
                ds_filtered = ds_highres.sel(ping_time=slice(start_time, end_time))

                # Check ping_time length
                ping_times = ds_filtered.ping_time.values
                if len(ping_times) < 2:
                    return jsonify({
                        "error": f"Selected time range too narrow. Found only {len(ping_times)} ping_time point(s). Please expand the range."
                    }), 400
                if pd.Timestamp(ping_times[0]) == pd.Timestamp(ping_times[-1]):
                    return jsonify({
                        "error": "Selected time range has identical ping_time values. Please expand the range."
                    }), 400

            except Exception as e:
                app.logger.error(f"Error filtering time range: {str(e)}")
                return jsonify({"error": f"Invalid time range or data extraction: {str(e)}"}), 400
        else:
        ...
        filename = f"echogram_{channel_index}_{vmin}_{vmax}_{start_time.strftime('%Y%m%d%H%M')}_{end_time.strftime('%Y%m%d%H%M')}"
        output_path = os.path.join(OUTPUT_DIR, f"{filename}.html")
        layout.save(output_path)  # Save echogram as interactive HTML
